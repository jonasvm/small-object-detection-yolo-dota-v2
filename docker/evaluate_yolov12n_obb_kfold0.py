# evaluate_yolov12n_obb_kfold0.py
import os
import gc
import time
import numpy as np
...
# ===== Caminhos (Linux) =====
model_path = '/home/jonasvm/docker-images/meus_resultados/obb/train72/weights/best.pt'
results_csv = '/home/jonasvm/docker-images/meus_resultados/obb/train72/results.csv'
data_yaml  = '/home/jonasvm/docker-images/dota_dataset_v15/dataset_config_fold0.yaml'
...
def measure_cpu_latency(model_file, imgsz=1024, runs=20, warmup=3):
    """
    Recarrega o modelo em CPU e mede a latência média (ms) num tensor aleatório.
    Recarregar evita o erro 'Inference tensors do not track version counter'
    após ter rodado .val() na mesma instância. (Bug conhecido)
    """
    # Garantir limpeza antes de criar a nova instância
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()

    m = YOLO(model_file)  # nova instância
    img = np.random.randint(0, 255, (imgsz, imgsz, 3), dtype=np.uint8)

    # Warm-up (não medir)
    for _ in range(warmup):
        _ = m.predict(img, device='cpu', imgsz=imgsz, verbose=False)
    ...
    # Libera a instância usada apenas para latência
    del m
    gc.collect()

    return (sum(times) / len(times)) * 1000.0  # ms

# ===== Carregar modelo (instância para as avaliações) =====
...
# ===== Latência média em CPU em instância separada =====
latency_ms = measure_cpu_latency(model_path, imgsz=IMG_SZ, runs=20, warmup=3)

# ===== Imprimir Tabela Markdown =====
print("\n### 🧪 Resultados — YOLOv12n-OBB (imgsz 1024)")
print(
...
print("\n[OK] Avaliações concluídas: train, val e test (usando val(split=...)). Latência medida em instância separada (CPU).")
